...
void ArenaAllocator_push(ArenaAllocator *this, Size size) {
    ArenaAllocator_Chunk *chunk = Allocator_malloc(this->provider, sizeof(ArenaAllocator_Chunk) + MEMALIGN(size) + HEADER_SIZE * ARENA_ALLOCATOR_PREDICTED_HEADERS);
...
void *ArenaAllocator_malloc(void *vthis, Size size) {
    ...
    header->size = size;
    this->chunk->offset = MEMALIGN(offset + size + HEADER_SIZE);
